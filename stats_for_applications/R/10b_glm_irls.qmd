...
-   For the weight matrix we need:
    -   $\partial \mu / \partial X\beta$, where $X \beta$ is often
        ...
        $\mu = g^{-1}(\eta)$, so $\mu' = (g^{-1})'$). This is the function that `stats::glm` refers to as `mu.eta`.
...
## GLM's Dispersion Parameter

For some distributions, the dispersion parameter is assumed to be 1. For others, it is technically unknown and may be considered a "nuisance parameter" in the frequntist framework. For example, in linear regression $\hat{\sigma}^2$ in $\hat{\beta} \sim N_p(\beta, \sigma^2 (X^\top X)^{-1})$ (i.e., part of the variance of $\beta$) is considered a nuisance parameter because it is not the quantity of interest but is needed for hypothesis testing of the quantity of interest. In fact, $\sigma^2$ is the dispersion parameter for the Gaussian GLM.  
...
fit_glm_irls <- function(X, y, fam, tol = 1e-8, maxit = 25) {
  ...
  for (it in 1:maxit) {
    ...
    # are we there yet?
    diff <- sqrt(crossprod(beta - betaold)) # l2 norm, ie norm(..., type="2")
    cat(paste0("[",it,"/",maxit,"]: l2 norm = ", diff, "\n"))
    if (diff < tol) break
  }
  #
