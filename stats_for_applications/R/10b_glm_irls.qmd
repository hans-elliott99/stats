...
The exponential family has distribution of the form (in canonical form here):

$$
f_\theta(y) = \exp(~~ \frac{y \theta - b(\theta)}{a(\phi)} + c(y, \phi) ~~)
$$
Where $b$ is known and is the cumulant generating function, $c$ is known, and is essentially a normalizing constant that we can drop when we maximize the likelihood, and typically $a(\phi) = \phi/w$, for some prior weights $w$ (which you can think of as $1$ in most cases).
...
$g$ is the link function which maps the mean parameter $\mu$ from its domain to $\mathbb{R}$, so that it can be modeled by $X \beta$. The link must be invertible so that we can compute $E[Y|X] = \mu = g^{-1}(X \beta)$.  
...
### Example with Data: Logistic regression on the kyphosis dataset
...
#
# first, prep the outcome vector:
# left col is no. of successes, right col is no. of failures
ytrials <- cbind(yraw, size - yraw)
summary(glm(ytrials ~ xraw[, 2:ncol(xraw)], family = binomial(),
            weights = rep(100, 100)))
...
# 
# summary(glm(yraw ~ -1 + xraw, family = poisson(link = "log")))$coef
...
# ix <- which(testpetrol$PetrolPrice > 0.08 & testpetrol$PetrolPrice < 0.14)
# plot(testpetrol$PetrolPrice[ix], testpetrol$pred[ix],
#      type = "l",
#      main = "Modeled count of drivers killed",
#      xlab = "Petrol Price", ylab = "Count"
#      ); points(seatbelts$PetrolPrice, seatbelts$DriversKilled)
# 
# # what if gas was free?
